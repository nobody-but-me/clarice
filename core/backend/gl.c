...
enum window_mode g_window_mode;
...
{
	if(g_window_mode==windowed_mode)
		gl(set_window_mode)(fullscreen_mode);
	else
		gl(set_window_mode)(windowed_mode);
	return;
}
...
int gl(init)(const enum window_mode window_mode_)
{
	...
	else if (g_window_mode==fullscreen_mode)
	{
		g_current_window_height=g_fullscreen_height;
		g_current_window_width=g_fullscreen_width;
...
{
	if(g_window_mode==fullscreen_mode)
		gl(toggle_window_fullscreen)();
	glfwTerminate();
	return;
}
